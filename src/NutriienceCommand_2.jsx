...
:root{
  --bg:#0e1729;--surface:#16213a;--surface2:#1e2a3e;--surface3:#243248;
  --border:#263550;--border2:#2e3f5c;
  --text:#e8f0fb;--text2:#7a9bbf;--text3:#4a6480;
  --accent:#1878a8;--blue:#4da6d6;
  --red:#ff6b7a;--yellow:#ffd166;--green:#2dd4a8;--brand-blue:#1878a8;--brand-gray:#8a9099;
  --red-bg:rgba(255,107,122,0.08);--yellow-bg:rgba(255,209,102,0.08);--green-bg:rgba(45,212,168,0.08);--blue-bg:rgba(77,166,214,0.09);
}
...
.header{display:flex;align-items:center;justify-content:space-between;padding:12px 24px;background:var(--surface);border-bottom:1px solid var(--border);position:sticky;top:0;z-index:200;gap:12px;flex-wrap:wrap}
...
.admin-toggle{background:var(--surface2);border:1px solid var(--border2);color:var(--text2);padding:6px 10px;border-radius:6px;font-size:11px;font-weight:600;letter-spacing:.3px;transition:all .15s;white-space:nowrap;cursor:pointer}
.admin-toggle:hover{border-color:#4da6d6;color:#4da6d6}
.admin-toggle.active{background:rgba(77,166,214,.12);border-color:#4da6d6;color:#4da6d6}

/* NAV */
.nav{display:flex;gap:2px;padding:6px 24px;background:var(--surface);border-bottom:1px solid var(--border);overflow-x:auto}
...
.nav-btn.active{background:var(--surface2);color:#4da6d6;border:1px solid rgba(77,166,214,.35)}
...
/* PIN SCREEN */
.pin-screen{position:fixed;inset:0;background:var(--bg);display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;gap:0}
...
function AdminPanel({ d, onSave, onClose }) {
  ...
  const F = ({ label, path, type = "number" }) => (
    <div className="field"><label className="flabel">{label}</label>
      <input className="finput" type={type} value={path.split(".").reduce((o, k) => o?.[k], draft) ?? ""} onChange={e => set(path, type === "number" ? num(e.target.value) : e.target.value)} />
    </div>
  );
            ...
            <div>
              ...
              {draft.customers.map((c, i) => (
                <div key={c.id} style={{ marginBottom: 16, padding: 14, background: "var(--surface2)", borderRadius: 10, border: "1px solid var(--border)" }}>
                  ...
                  <div className="fg2">
                    <F label="Name" path={`customers.${i}.name`} type="text" />
                    <F label="Revenue MTD" path={`customers.${i}.revenue_mtd`} />
                    <F label="Revenue YTD" path={`customers.${i}.revenue_ytd`} />
                    <F label="GP$ MTD" path={`customers.${i}.gp_mtd`} />
                    <F label="GP$ YTD" path={`customers.${i}.gp_ytd`} />
                    <F label="GM%" path={`customers.${i}.gm`} />
                    <F label="Open A/R" path={`customers.${i}.arOpen`} />
                    <F label="Past Due A/R" path={`customers.${i}.arPastDue`} />
                    <F label="Avg Order Size" path={`customers.${i}.avgOrderSize`} />
                    <F label="SKUs" path={`customers.${i}.skus`} />
                    <F label="Manufacturers" path={`customers.${i}.manufacturers`} />
                    <F label="Open Issues" path={`customers.${i}.openIssues`} />
                    <F label="Pay Days (avg)" path={`customers.${i}.payDays`} />
                    <F label="Concentration %" path={`customers.${i}.concentration`} />
                  </div>
      ...
      <div className="app">
        <header className="header">
          <div className="brand">
            ...
            <div><div className="brand-name" style={{ color: "#1878a8", letterSpacing: "0.5px" }}>Nutriience</div><div className="brand-name" style={{ color: "#e8f0fb", fontSize: 12, fontWeight: 600, letterSpacing: "1px" }}>Command</div><div className="brand-sub">Decision Cockpit · {data.settings.employees} FTE</div></div>
          </div>
